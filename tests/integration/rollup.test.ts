...
describe('Rollup Schema and Logic Validation Tests', () => {
  it('validates 005_create_ping_log_hourly.sql schema', () => {
    const sql = fs.readFileSync(path.join(__dirname, '../../src/db/migrations/005_create_ping_log_hourly.sql'), 'utf8');
  ...
  it('validates 006_create_ping_log_daily.sql schema', () => {
    const sql = fs.readFileSync(path.join(__dirname, '../../src/db/migrations/006_create_ping_log_daily.sql'), 'utf8');
