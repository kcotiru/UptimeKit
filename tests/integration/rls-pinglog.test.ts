...
describe('RLS PingLog and Resource Policies Verification', () => {
  it('validates 011_enable_rls_pinglog.sql policy names and team isolation', () => {
    const sql = fs.readFileSync(path.join(__dirname, '../../src/db/migrations/011_enable_rls_pinglog.sql'), 'utf8');
  ...
  it('validates 013_enable_rls_users.sql policy names', () => {
    const sql = fs.readFileSync(path.join(__dirname, '../../src/db/migrations/013_enable_rls_users.sql'), 'utf8');
