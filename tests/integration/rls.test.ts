...
describe('Row-Level Security Integration / Validation Tests', () => {
  it('validates 010_enable_rls_monitors.sql policy structure', () => {
    const sql = fs.readFileSync(path.join(__dirname, '../../db/migrations/010_enable_rls_monitors.sql'), 'utf8');
  ...
  it('validates 010 reverse migration', () => {
    const downSql = fs.readFileSync(path.join(__dirname, '../../db/migrations/down/010_drop_rls_monitors.sql'), 'utf8');
