...
describe('Soft Delete and Purge Integration Tests', () => {
  it('verifies deletion queue table and monitors soft-delete flags', () => {
    const monitorsSql = fs.readFileSync(path.join(__dirname, '../../db/migrations/003_create_monitors.sql'), 'utf8');
    const queueSql = fs.readFileSync(path.join(__dirname, '../../db/migrations/009_create_deletion_queue.sql'), 'utf8');
