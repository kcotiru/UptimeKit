...
describe('Saved Views Integration / Validation Tests', () => {
  it('validates 007_create_saved_views.sql schema and indices', () => {
    const sql = fs.readFileSync(path.join(__dirname, '../../src/db/migrations/007_create_saved_views.sql'), 'utf8');
  ...
  it('validates 012_enable_rls_views.sql policy configuration', () => {
    const sql = fs.readFileSync(path.join(__dirname, '../../src/db/migrations/012_enable_rls_views.sql'), 'utf8');
