...
describe('Partitioning Integration / Validation Tests', () => {
  it('validates 004_create_ping_log_raw.sql partitioned table structure', () => {
    const sql = fs.readFileSync(path.join(__dirname, '../../src/db/migrations/004_create_ping_log_raw.sql'), 'utf8');
  ...
  it('validates partition-manager.sql helper functions exist', () => {
    const sql = fs.readFileSync(path.join(__dirname, '../../src/db/scripts/partition-manager.sql'), 'utf8');
